...
- 本设计尚未实施；
- 当前用户、金币、签到和站内消息仍存于 `PokerServer/data.json`；
- 已完成牌谱仍以 JSON Lines 追加到 `PokerServer/hands.jsonl`；
- 反馈仍以 JSON Lines 追加到 `PokerServer/feedback.jsonl`；
- 活跃房间及牌局状态仍只存在进程内的 `runtime.roomGames`；
- PM2 可以在进程退出后自动启动服务，但不能恢复内存牌局；
- 当前本地数据规模很小，旧数据导入 SQLite 本身风险较低；
- 真正的重构重点不是“把 JSON 换成数据库文件”，而是金币事务、买入托管、幂等结算、牌局快照和启动恢复。

本文档是后续实现、评审、测试、部署和回滚的约束。实际实施应分批进行，不应在一个未经充分验证的提交中同时改完所有持久化和游戏流程。
