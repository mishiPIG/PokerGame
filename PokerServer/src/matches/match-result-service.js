...
function buildRanking(game, winnerId, prize) {
    if (game.roomType === 'cash') {
        const cur = game.players.map(p => ({ userId: p.userId, username: p.username, displayName: p.displayName || p.username, net: (p.chips || 0) - (p.buyIn || 0) }));
        const vac = (game.vacatedPlayers || []).map(v => ({ userId: v.userId, username: v.username, displayName: v.displayName || v.username, net: (v.chips || 0) - (v.buyIn || 0) }));
        const covered = new Set([...cur, ...vac].map(r => r.userId));
        const hist = (game.statsHistory || []).filter(h => !covered.has(h.userId))
            .map(h => ({ userId: h.userId, username: h.username, displayName: h.displayName || h.username, net: h.net }));
        return [...cur, ...vac, ...hist].sort((a, b) => b.net - a.net)
            .map((r, i) => ({ rank: i + 1, userId: r.userId, username: r.username, displayName: r.displayName, net: r.net, unit: '筹码' }));
    }
    const fee = game.config.buyIn || 0;
    const order = [];
    const w = game.players.find(p => p.userId === winnerId);
    if (w) order.push({ userId: w.userId, username: w.username, displayName: w.displayName || w.username, net: (prize || 0) - fee });
    (game.statsHistory || []).slice().reverse().forEach(h => order.push({ userId: h.userId, username: h.username, displayName: h.displayName || h.username, net: -fee }));
    return order.map((r, i) => ({ rank: i + 1, userId: r.userId, username: r.username, displayName: r.displayName, net: r.net, unit: '金币' }));
}
...
function sendMatchResult(roomId, title, ranking) {
    ...
    ranking.forEach(r => {
        const sign = r.net >= 0 ? '+' : '';
        const line = ranking.map(x => `${x.rank}. ${x.displayName || x.username} ${x.net >= 0 ? '+' : ''}${x.net}`).join('\n');
