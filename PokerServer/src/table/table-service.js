...
function createTableService({ io, db, stats, equity, Deck, HandEvaluator, crypto, config, runtime }) {
    ...
    let seatService;
    let pokerService;
    const sngMatchService = createSngMatchService({ io, db, roomGames, sngPrize, PHASES: config.PHASES, hooks: { broadcastState: (...args) => pokerService.broadcastState(...args), broadcastRoomList, buildRanking, sendMatchResult } });
