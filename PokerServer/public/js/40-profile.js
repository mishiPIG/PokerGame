...
async function openInbox() {
    ...
    list.innerHTML = `<div class="hist-empty">${L('加载中…', 'Loading…')}</div>`;
    const msgs = await fetchMessages();
...
async function openCheckin() {
    ...
    body.innerHTML = `<div class="hist-empty">${L('加载中…', 'Loading…')}</div>`;
    const st = await fetchCheckinStatus();
    if (!st) { body.innerHTML = `<div class="hist-empty">${L('加载失败，请重试', 'Failed to load, try again')}</div>`; return; }
    renderCheckin(st);
}
function renderCheckin(st) {
    ...
    const grid = st.rewards.map((r, i) => {
        ...
        return `<div class="ci-day${claimed ? ' claimed' : ''}${isToday ? ' today' : ''}">
            <div class="cd-d">${L(`第${day}天`, `Day ${day}`)}</div><div class="cd-r">+${r}</div></div>`;
    }).join('');
    body.innerHTML = `
        <div style="text-align:center;font-size:13px;opacity:.8">${L('连续签到得更多，断签重置 · 当前连签', 'Longer streaks earn more; a miss resets it · Current streak')} <b style="color:#f4d35e">${doneDays}</b> ${L('天', 'days')}</div>
        <div class="ci-grid">${grid}</div>
        <button class="ci-claim" id="ci-claim-btn" ${st.claimed ? 'disabled' : ''} onclick="doCheckin()">
            ${st.claimed ? L('今日已签到 ✓', 'Checked in today ✓') : L('签到领取 +', 'Check in +') + st.todayReward}
        </button>`;
}
async function doCheckin() {
    ...
    try {
        ...
        body.innerHTML = `<div style="text-align:center;padding:24px 8px">
            ...
            <div style="font-size:18px;font-weight:bold;margin-top:8px">${L('签到成功！', 'Checked in!')}+${data.reward} 🪙</div>
            <div style="opacity:.75;margin-top:6px">${L(`已连续签到 ${data.streak} 天，明天再来～`, `${data.streak}-day streak — come back tomorrow`)}</div>
            <div style="margin-top:10px;color:#f4d35e">${L('当前金币', 'Coins')}：${myGold.toLocaleString()}</div></div>`;
        refreshCheckinDot();
...
function renderDisplayNameSection() {
    ...
    const hint = left ? L(`下次可修改：${new Date(Date.now() + left).toLocaleString()}`, `Next change: ${new Date(Date.now() + left).toLocaleString()}`) : L('可每 24 小时修改一次', 'Can change once every 24h');
    box.innerHTML = `<div class="pi-section-title">${L('显示名称', 'Display name')}</div>
        <div class="pi-name-form"><input id="display-name-input" class="be-input" maxlength="16" value="${escapeHtml(myDisplayName || myUsername || '')}" aria-label="${L('显示名称', 'Display name')}">
        <button class="mini-btn" onclick="saveDisplayName()"${left ? ' disabled' : ''}>${L('保存', 'Save')}</button></div>
...
async function renderEmailSection() {
    ...
    box.innerHTML = `<div style="opacity:.6;font-size:13px">${L('邮箱加载中…', 'Loading email…')}</div>`;
    let email = null;
    ...
    box.innerHTML =
        `<div class="pi-email-row"><span>📧 ${email ? escapeHtml(email) : `<span style="color:#f87171">${L('未绑定', 'Not set')}</span>`}</span>
            <button class="mini-btn" onclick="toggleEmailForm()">${email ? L('更换', 'Change') : L('绑定', 'Bind')}</button></div>
...
function toggleEmailForm() {
    ...
    f.style.display = '';
    f.innerHTML =
        `<input id="be-email" class="be-input" type="email" placeholder="${L('新邮箱', 'New email')}">
         <button class="mini-btn" onclick="emailSendCode()">${L('发送验证码', 'Send code')}</button>
         <div id="be-step2" style="display:none;margin-top:6px">
            <input id="be-code" class="be-input" placeholder="${L('6 位验证码', '6-digit code')}" inputmode="numeric" maxlength="6">
            <button class="mini-btn" onclick="emailVerify()">${L('确认', 'Confirm')}</button>
         </div>
...
function renderProfileAvatars() {
    ...
    if (!g) return;
    g.innerHTML = `<div class="avatar-opt none ${!myAvatar ? 'sel' : ''}" onclick="setAvatar(null)">${L('无', 'None')}</div>`
...
async function loadStats(mode) {
    ...
    box.innerHTML = `<div class="hist-empty">${L('加载中…', 'Loading…')}</div>`;
    try {
        const token = localStorage.getItem('token');
    ...
    } catch { box.innerHTML = `<div class="hist-empty">${L('读取战绩失败', 'Failed to load stats')}</div>`; }
}
function renderStatsPage(s) {
    const box = document.getElementById('stats-body2');
    if (!s.totalHands) { box.innerHTML = `<div class="hist-empty">${L('还没有牌谱数据，打几手就有了', 'No hand data yet — play a few hands')}</div>`; return; }
    const netCol = s.net >= 0 ? '#4ade80' : '#f87171';
    const afTxt = s.af >= 99 ? '∞' : s.af;
    const cards = [
        [L('总手数', 'Hands'), s.totalHands, ''],
        [L('总盈亏', 'Net P/L'), (s.net >= 0 ? '+' : '') + s.net.toLocaleString(), netCol],
        [L('最大赢取', 'Biggest win'), s.biggestWin.toLocaleString(), ''],
        [L('VPIP 入池率', 'VPIP'), s.vpip + '%', '', L('自愿入池：翻前主动跟/加的比例', 'Voluntarily put money in preflop')],
        [L('PFR 翻前加注', 'PFR'), s.pfr + '%', '', L('翻前主动加注的比例', 'Preflop raise %')],
        [L('3-Bet 率', '3-Bet'), s.threeBet + '%', '', L('面对加注再加注的比例', '3-bet % facing a raise')],
        [L('弃 3-Bet', 'Fold to 3B'), s.foldTo3bet + '%', '', L('开池后被 3bet 选择弃牌的比例', 'Fold to 3-bet after opening')],
        [L('C-Bet 持续下注', 'C-Bet'), s.cbet + '%', '', L('作为翻前加注者翻牌续攻的比例', 'Continuation bet % as PFR')],
        [L('AF 激进度', 'AF'), afTxt, '', L('翻后(下注+加注)/跟注', 'Postflop (bet+raise)/call')],
        [L('WTSD 摊牌率', 'WTSD'), s.wtsd + '%', '', L('见翻牌后走到摊牌的比例', 'Went to showdown %')],
        [L('W$SD 摊牌胜率', 'W$SD'), s.wsd + '%', '', L('摊牌中获胜的比例', 'Won at showdown %')],
        [L('平均思考', 'Avg. think'), s.avgThinkMs ? (s.avgThinkMs / 1000).toFixed(1) + 's' : '—', ''],
    ];
    box.innerHTML = `<div class="stat-grid">` + cards.map(([k, v, col, tip]) =>
        ...
        + `<div class="sec-title" style="margin-top:14px">${L('盈亏曲线（按手数）', 'P/L curve (by hands)')}</div>`
        + profitCurveSVG(s.curve);
}
// 盈亏曲线 SVG（自适应宽度）
function profitCurveSVG(curve) {
    if (!curve || curve.length < 2) return `<div class="hist-empty" style="padding:14px">${L('数据太少，暂无曲线', 'Not enough data for a curve')}</div>`;
