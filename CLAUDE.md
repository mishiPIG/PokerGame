...
- **当前代码存储**：SQLite 保存用户、钱包流水、牌谱和活跃比赛快照；实时计算仍在内存，重启后从 SQLite 恢复。数据库路径由 `POKER_DB_PATH` 指定并放在代码目录外。旧 `data.json`/JSONL 只用于首次迁移与观察期回滚。
- **迁移/备份必做**：换服务器时必须迁移 SQLite 一致性快照（不能在 WAL 模式下只复制主文件），并单独迁移 `secret.key`/`mail.json`。使用 `scripts/backup-sqlite.js` 生成并校验备份。
...
- **演进方向**：当前阶段使用 SQLite；单机写入成为瓶颈或需要多实例时再迁移 PostgreSQL，`database.js` 兼容门面保持稳定。
