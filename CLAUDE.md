...
10. **🌐 中英文语言切换（2026-08-28 用户提出）** —— 设置里加「语言 中文 / English」开关（localStorage 持久）。⚠️ 工作量不小 + 有个结构性坑：**服务端 emit 给玩家看的桌面文案（弃牌/加注/赢池…）目前全是中文硬编码，且是一条广播发给全桌**——同房两人想要不同语言，一条广播满足不了。建议分层做：①**先做纯客户端静态 UI**（登录/大厅/设置/按钮/标签/收件箱模板，抽成 zh/en 字典 + `t(key)`），收益最大、风险最小；②服务端桌面提示要么改成发**结构化 key+参数**让各客户端本地渲染（大改，可参考 `nameOf` 那种集中改法），要么这批先保持中文/双语。**别一上来就想全量翻译。**
