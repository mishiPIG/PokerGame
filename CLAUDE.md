...
10. **🌐 中英文语言切换（2026-08-28 用户提出；第一步 part1 已做，测试服）** —— **已做**：i18n 引擎 `public/js/03-i18n.js`（`I18N` zh/en 字典 + `t(key)` + `applyLang()` 扫 `data-i18n`/`data-i18n-ph`/`data-i18n-title` + `setLang()`，localStorage 持久、首访按 `navigator.language` 自动判），设置面板加「语言 中/English」开关；已接入**登录/注册/找回页 + 大厅入口卡 + 顶栏按钮 tooltip + 设置面板**。jsdom 验证中英切换 文本/placeholder/title/高亮/html.lang 全对。**待续（step1 剩余）**：创建房间表单、牌桌内、管理面板、收件箱、以及 JS 动态渲染的文案/toast（这些要把字符串包进 `t()`）。⚠️ 工作量不小 + 有个结构性坑：**服务端 emit 给玩家看的桌面文案（弃牌/加注/赢池…）目前全是中文硬编码，且是一条广播发给全桌**——同房两人想要不同语言，一条广播满足不了。建议分层做：①**先做纯客户端静态 UI**（登录/大厅/设置/按钮/标签/收件箱模板，抽成 zh/en 字典 + `t(key)`），收益最大、风险最小；②服务端桌面提示要么改成发**结构化 key+参数**让各客户端本地渲染（大改，可参考 `nameOf` 那种集中改法），要么这批先保持中文/双语。**别一上来就想全量翻译。**
